        ...
        while (true) {
            ...
            if (!special.includes(instr) && !comment.test(this.instructions[this.pc])) {
                ...
                if (nextInstruction) {
                    ...
                    if (reach.regExp.test(instr)) {
                    ...
                    } else if (increaseDurability.regExp.test(instr)) {
                        const a = this.getEntity(variables[0]);
                        const i = this.getItem(variables[1]);
                        i.values.push(a.value);
                    } else if (increaseDurabilityByPoints.regExp.test(instr)) {
                        const a = this.getEntity(variables[0]);
                        const i = this.getItem(variables[1]);
                        const points = Number(variables[2]);
                        i.values.splice(points, 0, a.value);
                    } else if (increaseDurabilityAlongside.regExp.test(instr)) {
                        const a = this.getEntity(variables[0]);
                        const i = this.getItem(variables[1]);
                        const b = this.getEntity(variables[2]);
                        i.values.splice(b.value, 0, a.value);
                    } else if (decreaseDurability.regExp.test(instr)) {
                        const i = this.getItem(variables[0]);
                        i.values.pop();
                    } else if (breaks.regExp.test(instr)) {
                        const i = this.getItem(variables[0]);
                        const a = this.getEntity(variables[1]);
                        i.values.splice(a.value, 1);
                    } else if (decreaseDurabilityByPoints.regExp.test(instr)) {
                        const i = this.getItem(variables[0]);
                        ...
                        i.values[a.value] = temp;
                    } else if (consume.regExp.test(instr)) {
                        this.getEntity(variables[0]);
                        const i = this.getItem(variables[1]);
                        i.values = [];
                    } else if (bless.regExp.test(instr)) {
                        this.getEntity(variables[0]);
                        const i = this.getItem(variables[1]);
